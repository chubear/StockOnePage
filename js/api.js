...
export function fetchDailyQuotes(ticker, signal) {
  return queryTable('md_cm_ta_dailyquotes', {
    ticker,
    start: fiveYearsAgo(),
    end: today(),
    page_size: 1500,
...
export function fetchCompanyProfile(ticker, signal) {
  return queryTable('bd_cm_companyprofile', {
    ticker,
    page_size: 1,
  }, signal);
}
...
export function fetchMonthSales(ticker, signal) {
  return queryTable('md_cm_fi_monthsales', {
    ticker,
    page_size: 12,
  }, signal);
}
...
export function fetchQuarterlyIncome(ticker, signal) {
  return queryTable('md_cm_fi_is_quarterly', {
    ticker,
    page_size: 8,
  }, signal);
}
...
export function fetchForeignTrading(ticker, signal) {
  return queryTable('md_cm_fd_foreigninsttrading', {
    ticker,
    page_size: 30,
  }, signal);
}
...
export function fetchTrustTrading(ticker, signal) {
  return queryTable('md_cm_fd_investmenttrusttrading', {
    ticker,
    page_size: 30,
  }, signal);
}
...
export function fetchBrokerTrading(ticker, signal) {
  return queryTable('md_cm_fd_brokertrading', {
    ticker,
    page_size: 30,
  }, signal);
}
...
export function fetchShareholderStructure(ticker, signal) {
  return queryTable('md_cm_fd_stockholderstructure', {
    ticker,
    page_size: 12,
  }, signal);
}
